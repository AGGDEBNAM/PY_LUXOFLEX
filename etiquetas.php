...
<?php
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                if (isset($_POST["edit_etiqueta"])) {
                    ...
                    exit();
                } elseif (isset($_POST["delete_etiqueta"])) {
...
<body>
    ...
    <?php
    global $ruta;
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["insert_etiqueta"])) {
    ...
    if (isset($_POST["insert_etiqueta"])) {
        ...
        try {
            ...
            $sql_etiqueta = "INSERT INTO `etiqueta` (`tipo_forma`, `medida_ancho`, `medida_alto`,
                `medida_circunferencia`, `material_etiqueta`, `laminado`, `material_aplicacion`,
                `cantidad_de_colores`, `colores`, `disenio`, `id_contacto`)
                VALUES ('$tipo_forma', '$medida_ancho', '$medida_alto',
                '$medida_circunferencia', '$material_etiqueta','$laminado', '$material_aplicacion',
                '$cantidad_de_colores', '$colores', '$ruta', '$id_contacto')";
    ...
    <div class="card-perfil">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" enctype="multipart/form-data">
            ...
            <div class="card-etiqueta">
                <label for="medida_ancho">Media del ancho</label>
                <input type="text" name="medida_ancho" id="medida_ancho" required>
            </div>
            <div class="card-etiqueta">
                <label for="medida_alto">Media del alto</label>
                <input type="text" name="medida_alto" id="medida_alto" required>
            </div>
            <div class="card-etiqueta">
                <label for="medida_circunferencia">Media de la circunferencia</label>
            ...
            <div class="card-etiqueta">
                ...
                <select name="material_etiqueta" id="material_etiqueta">
                    ...
                    <option value="transferencia termica">Transferencia termica</option>
                </select>
            </div>
            <div class="card-etiqueta">
            ...
            <tbody>
                ...
                if ($result_etiquetas->num_rows > 0) {
                    while ($row_etiqueta = $result_etiquetas->fetch_assoc()) {
                        ...
                        echo "<td>{$row_etiqueta['disenio']}</td>";
                        
                        echo "<td>";
                            echo "<form method='post' action='{$_SERVER['PHP_SELF']}'>";
                            echo "<input type='hidden' name='edit_id' value='{$row_etiqueta['id_etiqueta']}'>";
                            echo "<button type='submit' name='edit_etiqueta'>Editar</button>";
                            echo "</form>";

                        echo "<form method='post' action='{$_SERVER['PHP_SELF']}'>";
                            echo "<input type='hidden' name='delete_id' value='{$row_etiqueta['id_etiqueta']}'>";
                            echo "<button type='submit' name='delete_etiqueta'>Eliminar</button>";
                            echo "</form>";
                            echo "</td>";
                    }
                } else {
                    echo "<tr><td colspan='7'>No hay etiquetas registradas.</td></tr>";
                }
                ?>
            </tbody>
        </table>
